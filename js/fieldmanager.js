...
fm_add_another = function( $element ) {
 ...
 new Date($.now());
 var dt = new Date(); 
 var count = dt.getTime(); 
 
	$new_element.find('ul.fm-tab-bar li:first-child a').attr("href", "#tab-1-" + count);
	$new_element.find('.fm-group-inner div.wp-tabs-panel:first-child').attr("id", "tab-1-"+ count);

	$new_element.find('ul.fm-tab-bar li:nth-child(2) a').attr("href", "#tab-2-" + count);
	$new_element.find('.fm-group-inner div.wp-tabs-panel:nth-child(2)').attr("id", "tab-2-"+ count);

	$new_element.find('ul.fm-tab-bar li:nth-child(3) a').attr("href", "#tab-3-" + count);
	$new_element.find('.fm-group-inner div.wp-tabs-panel:nth-child(3)').attr("id", "tab-3-"+ count);

	$new_element.find('ul.fm-tab-bar li:nth-child(4) a').attr("href", "#tab-4-" + count);
	$new_element.find('.fm-group-inner div.wp-tabs-panel:nth-child(4)').attr("id", "tab-4-"+ count);

	$new_element.find('ul.fm-tab-bar li:nth-child(5) a').attr("href", "#tab-5-" + count);
	$new_element.find('.fm-group-inner div.wp-tabs-panel:nth-child(5)').attr("id", "tab-5-"+ count);


	$new_element.find('.fm-content_group .fm-tab-bar li:first-child a').attr("href", "#subtab-1-" + count);
	$new_element.find('.fm-content_group .fm-group-inner div.wp-tabs-panel:first-child').attr("id", "subtab-1-"+ count);

	$new_element.find('.fm-content_group .fm-tab-bar li:nth-child(2) a').attr("href", "#subtab-2-" + count);
	$new_element.find('.fm-content_group .fm-group-inner div.wp-tabs-panel:nth-child(2)').attr("id", "subtab-2-"+ count);

	$new_element.find('.fm-content_group .fm-tab-bar li:nth-child(3) a').attr("href", "#subtab-3-" + count);
	$new_element.find('.fm-content_group .fm-group-inner div.wp-tabs-panel:nth-child(3)').attr("id", "subtab-3-"+ count);	
...
$( document ).ready( function () {
	...
	$( '.fm-collapsible .fm-group-label-wrapper' ).live( 'click', function() {
		$( this ).parents( '.fm-group' ).first().children( '.fm-group-inner, .fm-repeatable_group .fm-tab-bar' ).toggle();
	...
	$( '.fm-collapsed' ).each( function() {
		$( this ).find( '.fm-group-inner, .fm-repeatable_group .fm-tab-bar' ).hide();
	} );
